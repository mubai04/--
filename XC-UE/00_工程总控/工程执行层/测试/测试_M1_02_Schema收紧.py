...
def _valid_samples() -> dict[str, dict]:
    return {
        ...
        "第三层补丁审计结构.json": {
            ...
            "rollback_sha256": "",
            "final_source_sha256": "",
            "last_complete_artifact_sha256": SHA256,
            ...
            "finished_at": "2026-06-24T00:00:01+08:00",
            "final_status": "",
            "approval": None,
            "final_decision": None,
            "revalidation": {
                "report_json": "运行记录/pytest/复验/检测报告.json",
                "report_sha256": SHA256,
                "failure_packet": "运行记录/pytest/复验/失败包.json",
                "candidate_sha256": SHA256,
                "accepted": True,
            },
            "lineage": {
            ...
            "operation": "REPLACE",
            "anchor": "pytest anchor",
            "expected_text": "pytest anchor",
            "replacement_text": "pytest replacement",
            "append_text": "",
            "position": "AFTER",
            "reason": "pytest",
            "acceptance_conditions": ["pytest"],
            "risks": ["pytest"],
            "候选正文": "运行记录/pytest/候选正文.md",
            "unified_diff": "运行记录/pytest/补丁.diff",
            "正式正文保持不变": True,
            "decision": "",
            "decision_reason": "",
            "approver": "",
...
def _invalid_cases(name: str, valid: dict) -> list[dict]:
    ...
    elif name == "第三层补丁审计结构.json":
        selected = [
            _drop_path(valid, ("candidate_sha256",)),
            _set_path(valid, ("schema_version",), "xcue.l3-patch-audit/1.0"),
            _set_path(valid, ("final_status",), "DONE"),
            _set_path(valid, ("rollback_attempted",), "yes"),
            _set_path(valid, ("source_sha256",), "A" * 64),
            _set_path(valid, ("operation",), "GENERATE"),
            _set_path(valid, ("unexpected",), "blocked"),
            _set_path(valid, ("final_decision",), "apply"),
        ]
