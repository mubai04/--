...
def 加载路由规则(path: Path) -> 路由规则集:
    ...
    routes = raw["routes"]
    rules = [_解析规则(item, index, version, digest) for index, item in enumerate(routes)]
    _校验冲突(rules)
    _校验显式同优先级歧义(routes)
...
def _解析规则(raw: Any, index: int, version: str, digest: str) -> 路由规则:
    ...
    return 路由规则(rule_id=rule_id, version=version, hash=digest, keywords=keywords, target=target)
