...
from commercial_gate_rules import evaluate_commercial_gates
...
def compute_linear_result(extraction: dict[str, Any], weight_set: WeightSet) -> dict[str, Any]:
    ...
    commercial_gate_result = evaluate_commercial_gates(extraction["gates"], input_type)
    if commercial_gate_result["status"] == "证据不足":
        final_level = "证据不足"
        cap_reasons.append("关键商业门证据不足：" + "、".join(commercial_gate_result["missing_gates"]))
    elif commercial_gate_result["status"] == "未通过" and final_level in {"高", "中"}:
        final_level = "低"
        cap_reasons.append("关键商业门未过，八维高分不得补偿：" + "、".join(commercial_gate_result["failed_gates"]))
    ...
    return {
        ...
        "weight_config_sha256": sha256_text(json.dumps({"weights":weight_set.weights,"bias":weight_set.bias,"thresholds":weight_set.thresholds,"critical_axes":list(weight_set.critical_axes)}, ensure_ascii=False, sort_keys=True, separators=(",",":"))),
        "commercial_gate_result": commercial_gate_result,
